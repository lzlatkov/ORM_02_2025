...
from main_app.validators import validate_range, validate_release_year
...
class VideoGame(models.Model):
    ...
    rating = models.DecimalField(max_digits=2, decimal_places=1, validators=[validate_range])
...
class Programmer(models.Model):
    ...
    def get_projects_with_technologies(self) -> QuerySet['Project']:
...
class Task(models.Model):
    ...
    def ongoing_high_priority_tasks(cls):
        return cls.objects.filter(priority='High', is_completed=False, completion_date__gt=models.F("creation_date"))
...
class Exercise(models.Model):
    ...
    def get_short_and_easy_exercises(cls):
        return cls.objects.filter(difficulty_level__lt=5, duration_minutes__lt=15,)
