...
def filter_authors_by_birth_year(year_one, year_two):
    authors = Author.objects.filter(birth_date__year__gte=year_one,
                                    birth_date__year__lte=year_two).order_by("-birth_date")

    return '\n'.join(f"{a.birth_date}: {a.first_name} {a.last_name}" for a in authors)


def change_reviewer_name(reviewer_name: str, new_reviewer_name: str):
